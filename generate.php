...
if($dept_array[$ms_row[0]]) {
	...
	echo '<b>OPEN:</b> ' . $ms_row[1] . ' - ' . $ms_row[2] . '<br>';
	...
	echo '<b>CLOSED:</b> ' .$closed_name . ' - ' . $closed_total . '<br>';

// SETUP THE JOURNAL COUNTS
// Query Database
//	$journal_result = mssql_query("SELECT COUNT(HEATSeq) AS NumJourn FROM Journal WHERE EntryDate = '$today_date' AND Tracker = '$ms_row[1]'");
//      $journal_row = mssql_fetch_row($journal_result);
// Query Database, updated closed ticket count
//	$journal_mysql_result = mysql_query("UPDATE `employees` SET `journals` = '$journal_row[0]' WHERE `name` = '$ms_row[1]'") or die('Query failed: ' . mysql_error());
//	echo '<b>JOURNALS:</b> ' . $ms_row[1] . ' - ' . $journal_row[0] . '<br>';
// SETUP THE TRANS OUT
// Query Database
//	$transout_result = mssql_query("SELECT COUNT(CallID) AS TransitionedOut FROM Asgnmnt WHERE DateAssign = '$today_date' AND AssignedBy = '$ms_row[1]' AND (AssignedBy <> Assignee)");
//	$transout_result = mssql_query("SELECT COUNT(CallID) AS TransitionedOut FROM Asgnmnt WHERE DateAssign = '$today_date' AND AssignedBy = '$ms_row[1]' AND AssignedBy <> Assignee AND Assignee NOT IN ('HARDWARE','CAR')");
//        $transout_row = mssql_fetch_row($transout_result);
// Query Database, updated trans count
//	$transout_mysql_result = mysql_query("UPDATE `employees` SET `trans_out` = '$transout_row[0]' WHERE `name` = '$ms_row[1]'") or die('Query failed: ' . mysql_error());
//	if ($transout_row[0] == ""){
//	$transout_mysql_result = mysql_query("UPDATE `employees` SET `trans_out` = '0' WHERE `name` = '$ms_row[1]'") or die('Query failed: ' . mysql_error());
//	}
//	echo '<b>TRANS OUT:</b> ' . $ms_row[1] . ' - ' . $transout_row[0] . '<br><hr>';
...
// AprilFools
// $ig_pos_count = $ig_pos_count + mt_rand(20,243);
